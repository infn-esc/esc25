  ...
  for (int it = 0; it < NIter; ++it) {
    ...
    for (int i = 0; i < N; ++i) {
      if (particles.z[i] != particles.x[i] + particles.y[i]) {
        std::cerr << "Result is incorrect!" << particles.z[i] << " "
                  << particles.x[i] + particles.y[i] << std::endl;
  ...
  for (int it = 0; it < NIter; ++it) {
    ...
    for (int i = 0; i < N; ++i) {
      if ((particlesVec.z[i] != particlesVec.x[i] + particlesVec.y[i])) {
        std::cerr << "Result is incorrect!" << particlesVec.z[i] << " "
                  << particlesVec.x[i] + particlesVec.y[i] << std::endl;
        return 1;  // Return an error code
      }
    }
  }
