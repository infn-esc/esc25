...
class CachingAllocator {
 public:
  ...
  void* allocate(std::size_t size) {
    ...
    if (binIndex <=
        MAX_BINS) {  // if the binIndex is below the MAX_BINS then we
      ...
      // if requested allocxation is too big --> return bad_alloc
      throw std::bad_alloc();
    }
  }
  ...
  void free() {
    ...
    for (auto const& bin : live_blocks) {
 ...
 private:
  struct
      MemoryBlock {  // this class is used to represent blocks we are going to
                     // allocate
